...
std::optional<Halfedge_Mesh::EdgeRef> Halfedge_Mesh::flip_edge(Halfedge_Mesh::EdgeRef e) {
    ...
    HalfedgeRef h_2to3 = h->next();
    HalfedgeRef h_3to1 = h_2to3->next();
    HalfedgeRef h_1to4 = h_twin->next();
    HalfedgeRef h_4to2 = h_1to4->next();
    // 顶点
    VertexRef v1 = h->vertex();
    VertexRef v2 = h_twin->vertex();
    VertexRef v3 = h_3to1->vertex();
    VertexRef v4 = h_4to2->vertex();
    // 面
    FaceRef f1 = h->face();
    FaceRef f2 = h_twin->face();
    // 重新连接各基本元素
    auto current_halfedge = h;

    auto ModifyNextHalfEdge = [&current_halfedge](HalfedgeRef next,FaceRef face)
    {
        current_halfedge->set_neighbors(next,current_halfedge->twin(),current_halfedge->vertex(),current_halfedge->edge(),face);
    };

    //face1 modify

    current_halfedge->set_neighbors(h_4to2,h_twin,v3,e,f1);
    current_halfedge = current_halfedge->next();
    ModifyNextHalfEdge(h_2to3,f1);
    current_halfedge = current_halfedge->next();
    ModifyNextHalfEdge(h,f1);
    
    f1->halfedge() = h;
    v2->halfedge() = h_2to3;

    //face2 modify
    current_halfedge = h_twin;
    current_halfedge->set_neighbors(h_3to1,h,v4,e,f2);
    current_halfedge = current_halfedge->next();
    ModifyNextHalfEdge(h_1to4,f2);
    current_halfedge = current_halfedge->next();
    ModifyNextHalfEdge(h_twin,f2);
...
std::optional<Halfedge_Mesh::VertexRef> Halfedge_Mesh::split_edge(Halfedge_Mesh::EdgeRef e) {
    ...
    //create split edge
    EdgeRef split_edge_1 = new_edge();
    EdgeRef split_edge_2 = new_edge();

    //create split halfedge
    HalfedgeRef split_edge_1_h = new_halfedge();
    HalfedgeRef split_edge_1_h_twin = new_halfedge();
    HalfedgeRef split_edge_2_h = new_halfedge();
    HalfedgeRef split_edge_2_h_twin = new_halfedge();

    //create split face
    FaceRef face_123_1 = new_face();
    FaceRef face_123_2 = new_face();
    FaceRef face_124_1 = new_face();
    FaceRef face_124_2 = new_face();

    //create new edge
    EdgeRef new_edge_1 = new_edge();
    EdgeRef new_edge_2 = new_edge();
    new_edge_1->is_new = true;
    new_edge_2->is_new = true;

    //create new halfedge
    HalfedgeRef new_edge_1_h = new_halfedge();
    HalfedgeRef new_edge_1_h_twin = new_halfedge();
    HalfedgeRef new_edge_2_h = new_halfedge();
    HalfedgeRef new_edge_2_h_twin = new_halfedge();

    //set halfedges
    split_edge_1_h->set_neighbors(h_2to3,split_edge_1_h_twin,v,split_edge_1,face_123_1);
    split_edge_1_h_twin->set_neighbors(new_edge_2_h_twin,split_edge_1_h,v2,split_edge_1,face_124_1);
    split_edge_2_h->set_neighbors(new_edge_1_h,split_edge_2_h_twin,v1,split_edge_2,face_123_2);
    split_edge_2_h_twin->set_neighbors(h_1to4,split_edge_2_h,v,split_edge_2,face_124_2);

    new_edge_1_h->set_neighbors(h_3to1,new_edge_1_h_twin,v,new_edge_1,face_123_2);
    new_edge_1_h_twin->set_neighbors(split_edge_1_h,new_edge_1_h,v3,new_edge_1,face_123_1);
    new_edge_2_h->set_neighbors(split_edge_2_h_twin,new_edge_2_h_twin,v4,new_edge_2,face_124_2);
    new_edge_2_h_twin->set_neighbors(h_4to2,new_edge_2_h,v,new_edge_2,face_124_1);

    h_2to3->next() = new_edge_1_h_twin;
    h_2to3->face() = face_123_1;

    h_3to1->next() = split_edge_2_h;
    h_3to1->face() = face_123_2;

    h_1to4->next() = new_edge_2_h;
    h_1to4->face() = face_124_2;

    h_4to2->next() = split_edge_1_h_twin;
    h_4to2->face() = face_124_1;

    //set edge
    split_edge_1->halfedge() = split_edge_1_h;
    split_edge_2->halfedge() = split_edge_2_h;
    new_edge_1->halfedge() = new_edge_1_h;
    new_edge_2->halfedge() = new_edge_2_h;

    //set face
    face_123_1->halfedge() = h_2to3;
    face_123_2->halfedge() = h_3to1;
    face_124_1->halfedge() = h_4to2;
    face_124_2->halfedge() = h_1to4;
    ...
    v->is_new = true;
    v->pos = 0.5f * (v1->pos + v2->pos);
    v->halfedge() = split_edge_1_h;
...
void Halfedge_Mesh::loop_subdivide() {
    ...
    auto FindAdjacentVertex = [&vertex_count,&sum_vec](VertexRef v) 
    {
        HalfedgeRef h = v->halfedge();
        sum_vec += h->twin()->vertex()->pos;
        vertex_count++;
        h = h->twin()->next();
    ...
    auto AdjustEdgeVertex = [](EdgeRef e)
    {
        HalfedgeRef h = e->halfedge();
        ...
        e->new_pos = 0.375 * (v1->pos + v2->pos) + 0.125 * (v3->pos + v4->pos);
    };

    auto IsOldEdge = [](EdgeRef e) -> bool
    {
        VertexRef v1 = e->halfedge()->vertex();
        VertexRef v2 = e->halfedge()->twin()->vertex();

        if(e->is_new || (v1->is_new || v2->is_new)) return false;
        return true;
    };

    for(auto iter = vertices_begin();iter != vertices_end();iter ++)
    {
        FindAdjacentVertex(iter);
        if(vertex_count == 3)
            iter->new_pos = iter->pos * 0.4375f + sum_vec * 0.1875f;
        else
            iter->new_pos = iter->pos * 0.625f + sum_vec * 0.375f / float(vertex_count);
    ...
    {
        AdjustEdgeVertex(iter);
    }

    for(auto iter = edges_begin();iter != edges_end();iter++)
    {
        if(!IsOldEdge(iter)) continue;
        auto vertex = split_edge(iter);
        vertex.value()->new_pos = iter->new_pos;
    }

    for(auto iter = edges_begin();iter != edges_end();iter++)
    {
        bool old_new_connect = (iter->halfedge()->vertex()->is_new ^ iter->halfedge()->twin()->vertex()->is_new);
        if(iter->is_new && old_new_connect)
            flip_edge(iter);
    }

    auto iter_e = edges_begin();
    while(iter_e != edges_end())
    {
        if (iter_e->is_new)
            iter_e->is_new = false;
        if(IsOldEdge(iter_e))
        {
            auto current_iter_e = iter_e;
            iter_e ++;
            erase(current_iter_e);
        }
        else
            iter_e ++;
    }
